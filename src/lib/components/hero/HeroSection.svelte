<script lang="ts">
	...
	function typeRole() {
		...
		if (!isDeleting) {
			displayRole = current.slice(0, displayRole.length + 1);
			if (displayRole === current) {
				setTimeout(() => {
					isDeleting = true;
					typeRole();
				}, 2000);
		...
		setTimeout(typeRole, isDeleting ? 40 : typingSpeed);
	}
</script>
	...
	<div class="container hero-inner">
		...
		<div class="hero-content" class:visible>
			...
			<div class="hero-role">
				...
				<span class="role-cursor animate-blink">_</span>
			</div>

			<p class="hero-bio">
				Staff Frontend Engineer with <strong>6+ years</strong> of experience engineering performant, accessible,
				and visually striking web applications. From AI-powered platforms to end-to-end encrypted systems,
				I build software that pushes the boundaries of what's possible in the browser.
			</p>
			...
			<div class="hero-ctas">
				...
				<a
					href="/assets/Timilehin_Akinsanmi_Resume.pdf"
					target="_blank"
					rel="noopener"
					class="btn-secondary"
		...
		<!-- Right: Terminal -->
		<div class="hero-terminal" class:visible>
			<TerminalEmulator />
			<p class="terminal-hint">
				<kbd>↑</kbd><kbd>↓</kbd> history · type <code>help</code> to start
			</p>
		</div>
	</div>
...
<style>
	...
	:global([data-theme='light']) .terminal-hint {
		color: var(--text-secondary);
	}
